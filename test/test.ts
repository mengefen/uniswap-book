...
import { getBalance, toWei, fromWei } from './utils';
...
describe('Exchange', function () {
  ...
  it('it is deployed', async () => {
    expect(await exchange.deployed()).to.equal(exchange);
    expect(await exchange.name()).to.equal('Exchange');
    expect(await exchange.symbol()).to.equal('EXCH');
  ...
  describe('addLiquidity', async () => {
    ...
    describe('existing reserves', async () => {
      ...
      it('fails when not enough tokens', async () => {
        ...
        ).to.be.revertedWith('Insufficient token amount sent');
      });
    });
  });
  ...
  describe('removeLiquidity', async () => {
    ...
    it('removes some liquidity', async () => {
      ...
      ).to.equal('24.99993595909191646'); // 25 - gas fees
    ...
    it('removes all liquidity', async () => {
      ...
      expect(
        fromWei(userEtherBalanceAfter.sub(userEtherBalanceBefore)),
      ).to.equal('99.999949099058885125'); // 100 - gas fees
    ...
    it('pays for provided liquidity', async () => {
      ...
      expect(
        fromWei(userEtherBalanceAfter.sub(userEtherBalanceBefore)),
      ).to.equal('109.999949300233570275'); // 110 - gas fees
